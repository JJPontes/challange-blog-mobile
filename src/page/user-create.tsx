...
import { NativeStackNavigationProp } from '@react-navigation/native-stack'; // Adicionado para tipagem correta
    ...
    >
      ...
      >
        {({ handleChange, handleBlur, handleSubmit, values, errors, touched }) => (
          ...
          >
            <View className="bg-white rounded-md p-6 w-full max-w-[500px] self-center mx-4">
              ...
              >
                ...
                <Text className="text-gray-500 text-lg font-semibold ml-2">Voltar</Text>
              </TouchableOpacity>

              <Text className="text-2xl font-bold text-gray-900 mb-6">
                Cadastrar {roleType === 'professor' ? 'professor' : 'estudante'}
              </Text>
              ...
              <View className="flex-row bg-gray-200 rounded-md p-1 mb-4">
                {(['professor', 'aluno'] as const).map((type) => (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                    className={`flex-1 py-3 items-center rounded-lg ${roleType === type ? 'bg-blue-600' : ''}`}
                  >
                    <Text className={`text-base font-bold capitalize ${roleType === type ? 'text-white' : 'text-gray-500'}`}>
                      {type}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              <View className="mb-4">
                <Text className="text-md font-bold text-gray-700 mb-2 ml-1">Nome completo</Text>
                <TextInput
                  ...
                  value={values.name}
                />
                {touched.name && errors.name && <Text className="text-red-500 text-xs mt-1 ml-1">{errors.name}</Text>}
              </View>

              <View className="mb-4">
                <Text className="text-md font-bold text-gray-700 mb-2 ml-1">E-mail</Text>
                <TextInput
                  className="bg-gray-50 border border-gray-300 rounded-md p-4 text-base text-gray-800 focus:border-blue-500"
              ...
              <View className="mb-8">
                <Text className="text-md font-bold text-gray-700 mb-2 ml-1">Senha</Text>
              ...
              <TouchableOpacity 
                className="bg-blue-600 py-4 rounded-md items-center shadow-sm" 
