...
export default function Users() {
  ...
  if (selectedUser) {
    navigation.navigate(Routes.USER_CREATE_EDIT.name, { userData: selectedUser });
  }
};
      ...
      >
        <View className="p-6 my-10 mx-5 rounded-md bg-white shadow-sm">
          <View>
            ...
            <Text className="text-textGray text-md font-bold mt-2">Gerencie os acessos dos usuários.</Text>
          </View>

          <View className="my-6">
            <TouchableOpacity className="bg-primary h-8 rounded-md items-center
             justify-center flex-row px-4 w-[140px]"
              onPress={() =>
                navigation.navigate(Routes.USER_CREATE_EDIT.name, { userData: null })
              }>
              <Plus size={20} weight="bold" color="#fff" />
              <Text className="text-white font-semibold ml-2 text-lg">Novo usuário</Text>
            </TouchableOpacity>
          </View>

          <View className="flex-row pb-3 border-b border-gray-200">
            <Text className="flex-[1.5] text-slate-400 font-bold uppercase text-[10px]">Nome</Text>
            <Text className="flex-[2] text-slate-400 font-bold uppercase text-[10px]">Email</Text>
            <Text className="flex-1 text-center text-slate-400 font-bold uppercase text-[10px]">Cargo</Text>
            <View className="w-8" />
          </View>

          <View className="min-h-[300px]">
            {loading && users.length === 0 ? (
              <View className="py-20"><ActivityIndicator color="#4F46E5" size="large" /></View>
            ) : (
              users.map((item) => (
                <View key={item.id} className="flex-row items-center py-4 border-b border-gray-100">
                  ...
                  <View className="flex-1 items-center">
                    <View className={`${getRoleConfig(item.role_name).bg} px-2 py-1 rounded-md min-w-[75px] items-center`}>
                      <Text className={`${getRoleConfig(item.role_name).text} text-[10px] font-bold uppercase`}>
          ...
          <View className="flex-row items-center justify-between pt-6 border-t border-gray-100">
            <Text className="text-slate-500 text-md">Total: {apiResponse?.pagination?.total || 0}</Text>
            <View className="flex-row items-center">
              <Text className="mr-4 text-md">Pág. {apiResponse?.pagination?.currentPage || 1} de {apiResponse?.pagination?.totalPages || 1}</Text>
